...
import { updateEntity } from 'app/entities/maison/maison.reducer';
...
export function House({ id, libelle, description, identifiant, image, etat, proprietaire, signalant }) {
  ...
  const handleClick = () => {
    toast.success('Vous pouvez supprimer votre maison en toute sécurité après être entré en possession!');
  };
            ...
            <>
              {found && (
                <Stack direction="row" spacing={1}>
                  <Chip
                    label="Votre maison a été retrouvé! Veuillez consulter vos mails pour plus d'infos!"
                    variant="outlined"
                    color="success"
                    onClick={handleClick}
              ...
              <div className="flex flex-row pl-3 justify-center">
                {/* <Button tag={Link} to={`/maison/${id}`} color="info" size="sm" data-cy="entityDetailsButton">
